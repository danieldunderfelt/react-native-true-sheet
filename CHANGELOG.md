...
- New `TrueSheet.suspendAll()` / `TrueSheet.unsuspendAll()` static methods: `suspendAll` captures every sheet open at call time and suspends it (natively stepped aside, logically still presented); sheets presented afterwards are unaffected, so a modal flow can suspend the sheets beneath it and still open its own. `unsuspendAll` re-presents the captured sheets at their remembered detents without re-emitting present events. Independent of — and composable with — the `suspended` prop. No-ops on web. ([#XXX](https://github.com/lodev09/react-native-true-sheet/pull/XXX) by [@author](https://github.com/author))

### 🐛 Bug fixes

- **iOS**: A sheet presented on top of a closing modal (React Navigation modal route or RN `Modal`) now survives the modal's dismissal: the library detects that the sheet went down only because its presenter was torn out from under it, suppresses the spurious dismiss events, and silently re-presents it once the teardown settles — across full sheet stacks. An explicit `dismiss()` during the teardown still closes it for real. ([#XXX](https://github.com/lodev09/react-native-true-sheet/pull/XXX) by [@author](https://github.com/author))
- **iOS**: `present()` no longer fails with "No presenting view controller found" when the sheet's host view is detached from the window — e.g. under a full-screen modal (UIKit removes the underlying screen's view) or during that modal's close transition. Presenter resolution falls back to the key window and defers around in-flight transitions. ([#XXX](https://github.com/lodev09/react-native-true-sheet/pull/XXX) by [@author](https://github.com/author))
- **iOS**: The sheet backdrop no longer goes missing when a sheet presents right after another modal dismissal (a UIKit presentation-container race): the library now verifies the system dimming view after each present and detent change, re-asserts the dim policy through public API when it's absent, and installs its own backdrop as a last resort for always-dimmed sheets. ([#XXX](https://github.com/lodev09/react-native-true-sheet/pull/XXX) by [@author](https://github.com/author))
- **iOS**: Fabric view recycling no longer leaks sheet identity across screens: the module registry now unregisters recycled views correctly (a stale tag could resolve `dismiss()` calls onto an unrelated, reused sheet — closing it), a reused view no longer inherits its previous incarnation's still-presented controller (which cancelled the new sheet's present), and stale pending props/detents are cleared on recycle. Dismissing an already-gone sheet now resolves instead of rejecting with `SHEET_NOT_FOUND`. ([#XXX](https://github.com/lodev09/react-native-true-sheet/pull/XXX) by [@author](https://github.com/author))
- Teardown races no longer surface as errors from `present()`/`dismiss()`/`resize()`: unmounting mid-present settles the promise quietly, concurrent presents before the native view mounts all settle, and calls on an unmounted or torn-down sheet are warn-level no-ops instead of uncaught rejections. ([#XXX](https://github.com/lodev09/react-native-true-sheet/pull/XXX) by [@author](https://github.com/author))
...
- **iOS**: A sheet no longer presents without its content — an empty bar that flashes and dismisses itself. This happened when a navigation re-present raced the content teardown (e.g. dismissing a `transparentModal` route while a sheet opens on the next screen) or when an initial present landed before the container mounted. The present now waits for the content to attach and replays once it does. ([#XXX](https://github.com/lodev09/react-native-true-sheet/pull/XXX) by [@author](https://github.com/author))
